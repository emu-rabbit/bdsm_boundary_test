...
10. 產生 structured question bank JSON 與型別需要的衍生資料；細項的共用 `項目` 僅保存為 `sourceLabel`，並依 `主導側描述`、`配合側描述` 分別產生兩個方向性 `title` 與 `description`，不得將共用標籤直接顯示為題名。
...
- 細項的 `項目` 是來源辨識 metadata，不是方向別使用者題名；所有語系都必須為主導側與配合側各自維護 `title`，且標題需能單獨辨識互動方向。
