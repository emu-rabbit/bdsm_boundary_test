...
- **高重複作答流程**：分類與細項作答屬於數十至數百次的高重複流程，版面應以單頁可見為優先，並同時保有明確的目前位置、完成進度與返回上一題路徑。每題應保留分類視覺錨點、避免重複呈現同一方向語意，且把必填選項做成可快速辨識的緊湊圖示加短標籤；兩項必填答案完成後應立即保存、短暫確認後自動前往下一題，不使用額外的「下一題」按鈕。
