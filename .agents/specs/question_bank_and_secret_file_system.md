...
本機 UI 可以在 localStorage 中另存目前進行到哪一題、最近開啟的分類或列表排序等暫態資訊，但這些不屬於秘密檔案 JSON 本體，也不應出現在複製 JSON 或雲端檔案中。
