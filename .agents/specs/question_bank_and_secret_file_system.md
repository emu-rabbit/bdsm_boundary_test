...
細項分頁的 `項目` 是共用的來源辨識與管理標籤，不是方向別的正式題名。它可保留為題庫 metadata，但不得直接作為細項作答、結果頁或唯讀檢視的使用者可見標題。
...
每個 `DetailItem` 必須為 `active` 與 `passive` 各保存一份使用者可見 `title` 與 `description`。兩個方向的 `title` 必須能獨立說明由誰對誰進行何種互動，不得重用細項分頁的共用 `項目` 標籤；後者只保留為 `sourceLabel` 或等價 metadata。
...
- 不得把共用的試算表 `項目`／`sourceLabel` 當作使用者可見標題。
...
- 編輯模式的 Spotlight 以五個固定容量區塊作為設定入口，不另設「挑選重點」按鈕；使用者可編輯已登記格或最前方空格，刪除後後方項目依序前移，不得把它混成系統自動推薦或高分排行。
...
- 喜好排行榜：使用者完全手動選擇最多 5 個項目，顯示在結果頁作為 spotlight。可選候選包含分類本身與細項，但僅限已作答且喜好為 `like` 或 `love` 的項目；未作答項目不可被選擇。選擇視窗將 `love` 與 `like` 分群，並固定優先顯示 `love`。五格只能點擊已登記的格子或最前方第一個空格；刪除後後方項目依序前移。排行榜是使用者自行整理偏好，不得呈現為社交排行或鼓勵嘗試清單。
...
`hardNo` 絕對禁止不需要觸發額外保護流程，但 UI 可以用顏色、字重或其他視覺設計讓界線清楚可掃讀。閱覽總覽若分類層答案本身為 `hardNo`，仍須如實呈現；細項 `hardNo` 不在總覽逐項外顯，而由分類相簿標題附近、帶有項目數量的入口開啟 dialog 集中列出，使閱讀者在檢查分類前能先注意界線。此入口不得放進檔案身份摘要側欄。只有至少一個 `hardNo` 時才顯示入口與 dialog；零項時兩者都不渲染，也不得留下空白占位。進入單一分類的完整結果後，細項答案仍應如實顯示。`reluctant` 勉強配合的命名保留。
