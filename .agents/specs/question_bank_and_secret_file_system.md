...
- 分類大按鈕需包含分類圖片、分類層答案與細項完成度，且整張卡片可作為進入該分類編輯的入口；畫面需明確提示使用者可點選分類繼續編輯。
...
- 編輯模式需為 Spotlight 保留可辨識的固定區位與最多 5 項的容量提示；五個容量區塊是未來設定的互動入口，不另設「挑選重點」按鈕。即使挑選功能尚未接入，也不應把它混成系統自動推薦或高分排行。
