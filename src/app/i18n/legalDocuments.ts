    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          '雲端分享連結屬於持有連結即可讀取的方式。你可以自行評估並決定分享的對象、平台或空間，也應理解連結可能被再次轉傳。雲端快照建立後目前不能由一般使用者自行修改或刪除；如需依法行使權利，請依隱私權政策所列方式聯絡。',
        ],
      },
      {
        title: '七、第三方服務與連結',
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          '雲端分享內容：你主動建立分享連結時上傳的檔案快照，可能包含 BDSM 互動項目的經驗、興趣、界限與備註。',
      ...
      },
      {
        ...
        paragraphs: [
          ...
          '只有在你主動建立雲端分享連結時，該份快照與必要的防濫用資料才會傳送到雲端。',
        ],
      },
      {
        title: '五、雲端分享與可見範圍',
        paragraphs: [
          '雲端分享使用不可猜測的連結識別碼；任何取得完整連結的人都可能讀取快照。你可以自行評估並決定要把連結分享到哪些人、平台或空間，也應理解連結可能被再次轉傳。雲端快照建立後是唯讀版本，目前沒有一般使用者自行修改或刪除的功能。',
      ...
      },
      {
        title: '八、保存期間',
        paragraphs: [
          '本機資料保存到你在功能內刪除、清除瀏覽器儲存空間，或裝置環境移除為止。雲端分享快照與私有防濫用雜湊紀錄目前採長期保存，以維持連結與處理濫用；除非依法應刪除、服務停止，或我們另行公告保存政策。',
      ...
      },
      {
        ...
        paragraphs: [
          ...
          '如要行使權利或詢問隱私事項，請使用本頁所列電子郵件聯絡。若依法需保留資料、無法可靠辨識特定雲端快照，或請求會影響他人權利，我們會說明可處理的範圍。',
        ],
      },
      {
        title: '十一、未成年人與敏感內容',
...
export const zhHansLegal = {
  ...
  terms: {
    ...
    sections: [
      ...
      { title: '六、你的内容与云端分享', paragraphs: ['你保有自己填写内容原有的权利。为了建立、保存、读取及传递你主动选择的云端分享版本，你授权本站在提供该功能所必要的范围内处理该内容；此授权不代表本站取得内容所有权。', '云端分享链接属于持有链接即可读取的方式。你可以自行评估并决定分享的对象、平台或空间，也应理解链接可能被再次转发。云端快照建立后目前不能由一般使用者自行修改或删除；如需依法行使权利，请依隐私权政策所列方式联系。'] },
  ...
  privacy: {
    ...
    sections: [
      ...
      { title: '四、只留在本机的数据', paragraphs: ['一般作答与秘密档案默认保存在你的浏览器本机存储空间，不会因单纯填写而自动送上云端。清除浏览器数据、更换设备、使用私人浏览，或浏览器与设备故障，都可能使本机数据消失。', '只有在你主动建立云端分享链接时，该份快照与必要的防滥用数据才会传送到云端。'] },
      { title: '五、云端分享与可见范围', paragraphs: ['云端分享使用不可猜测的链接识别码；任何取得完整链接的人都可能读取快照。你可以自行评估并决定要把链接分享到哪些人、平台或空间，也应理解链接可能被再次转发。云端快照建立后是只读版本，目前没有一般使用者自行修改或删除的功能。', '我们不会出售你的内容，也不会把秘密档案用于广告个性化或建立 BDSM 身份标签。'] },
      ...
      { title: '八、保存期间', paragraphs: ['本机数据保存到你在功能内删除、清除浏览器存储空间，或设备环境移除为止。云端分享快照与私有防滥用散列记录目前采用长期保存，以维持链接与处理滥用；除非依法应删除、服务停止，或我们另行公告保存政策。', '短期频率限制数据通常约 48 小时后自动到期。Google Analytics 使用者层级与事件层级数据依本站设置及 Google 提供的控制保存，最长不超过 14 个月；汇总报表可能不受相同期间限制。客服往来依处理问题与法律需要保存。'] },
      { title: '九、安全措施', paragraphs: ['我们采取与数据敏感度相称的合理措施，例如传输加密、访问规则、应用程序验证、频率限制、最少数据事件及来源信号的密钥散列。不过，任何网络或存储方式都无法保证绝对安全。'] },
      { title: '十、你的权利', paragraphs: ['依适用法律，你可以请求查询或阅览、取得副本、补充或更正、停止收集处理或利用，以及删除你的个人数据。这些权利不因使用本站而被放弃；我们可能先要求足以确认请求与数据关联的信息。', '如要行使权利或询问隐私事项，请使用本页所列电子邮件联系。若依法需保留数据、无法可靠辨识特定云端快照，或请求会影响他人权利，我们会说明可处理的范围。'] },
...
export const jaLegal = {
  ...
  terms: {
    ...
    sections: [
      ...
      { title: '6. 利用者の内容とクラウド共有', paragraphs: ['入力した内容に関する権利は利用者に帰属します。利用者が選択したクラウド共有版の作成、保存、表示、送信に必要な範囲で、本サイトにその内容を処理する権限を付与します。これは所有権の移転ではありません。', '完全な共有リンクを持つ人は内容を閲覧できます。共有する相手、プラットフォーム、場所はご自身で評価して決めることができますが、リンクがさらに共有される可能性も理解してください。作成後のクラウドスナップショットは、現在、一般利用者が変更・削除できません。法令上の権利を行使する場合はプライバシーポリシー記載の方法でご連絡ください。'] },
  ...
  privacy: {
    ...
    sections: [
      ...
      { title: '4. 端末にのみ残るデータ', paragraphs: ['通常の回答と秘密ファイルはブラウザのローカルストレージに保存され、入力しただけではクラウドへ送信されません。ブラウザデータの消去、端末変更、プライベートブラウジング、故障などで失われる場合があります。', '自らクラウド共有リンクを作成したときに限り、そのスナップショットと必要な不正利用防止データがクラウドへ送られます。'] },
      { title: '5. クラウド共有と閲覧範囲', paragraphs: ['共有には推測困難な識別子を使いますが、完全なリンクを得た人は閲覧できます。共有する相手、プラットフォーム、場所はご自身で評価して決めることができますが、リンクがさらに共有される可能性も理解してください。スナップショットは読み取り専用で、現在は一般利用者向けの変更・削除機能がありません。', '内容を販売せず、広告のパーソナライズやBDSMの属性ラベル作成には使用しません。'] },
      ...
      { title: '8. 保存期間', paragraphs: ['端末内データは機能上で削除するか、ブラウザストレージまたは端末環境から消去されるまで残ります。クラウド共有スナップショットと非公開の不正利用防止ハッシュ記録は、リンク維持と不正利用対応のため現在は長期保存します。ただし、法令上削除が必要な場合、サービス終了、または別途保存方針を告知した場合を除きます。', '短期レート制限データは通常約48時間で失効します。Google Analyticsのユーザー・イベント単位データは設定とGoogleの管理機能に従い最長14か月で、集計レポートは同期間の対象外となる場合があります。問い合わせ記録は対応と法的必要性に応じて保存します。'] },
      ...
      { title: '10. 利用者の権利', paragraphs: ['適用法に従い、照会・閲覧、複製、追加・訂正、収集・処理・利用の停止、削除を請求できます。利用によってこれらを放棄することはありません。請求とデータの関係を確認するため必要な情報を求める場合があります。', '権利行使やプライバシーの問い合わせは本ページ記載のメールへご連絡ください。法令上保存が必要な場合、特定スナップショットを確実に識別できない場合、他者の権利に影響する場合は、対応可能な範囲を説明します。'] },
...
export const enLegal = {
  ...
  terms: {
    ...
    sections: [
      ...
      { title: '6. Your content and cloud sharing', paragraphs: ['You retain the rights you already have in what you enter. You give the Site permission to process that content only as needed to create, store, display, and transmit the cloud sharing copy you choose. This does not transfer ownership to the Site.', 'Anyone with the complete sharing link may read the copy. You may assess and choose the people, platforms, or spaces where you share it, while understanding that the link may be shared again. Cloud snapshots currently cannot be changed or deleted directly by an ordinary user. To exercise rights under applicable law, contact us as described in the Privacy Policy.'] },
  ...
  privacy: {
    ...
    sections: [
      ...
      { title: '4. Data that stays on your device', paragraphs: ['Ordinary answers and secret files are stored in your browser’s local storage by default and are not sent to the cloud merely because you fill them in. Clearing browser data, changing devices, private browsing, or browser or device failure may erase them.', 'A snapshot and necessary anti-abuse data are sent to the cloud only when you actively create a cloud sharing link.'] },
      { title: '5. Cloud sharing and visibility', paragraphs: ['Cloud shares use hard-to-guess link identifiers, but anyone with the complete link may read the snapshot. You may assess and choose the people, platforms, or spaces where you share it, while understanding that the link may be shared again. A snapshot is read-only and currently has no ordinary-user edit or delete control.', 'We do not sell your content or use secret files for personalized advertising or BDSM identity labels.'] },
