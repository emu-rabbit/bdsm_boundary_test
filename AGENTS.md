...
## Production Deployment Environment Boundary

GitHub `production` Environment 目前刻意不設定 required reviewer；`main` push 通過 workflow 的 test／build 與既有部署條件後可直接進行 production deployment。所有目前與未來 Agent 都必須遵守：

- 把「沒有 required reviewer」視為使用者已確認的 production 設定，不得自行補上 reviewer、approval gate 或其他人工審查規則。
- 不得因目前沒有人工 gate，就把一般程式修改、push 或監看授權擴張成未被要求的 production 部署授權；仍需依使用者當次要求與既有 workflow 範圍行動。
- 若未來 production run 出現 `Waiting`、`Pending approval`、`Review deployments` 或其他與本文件不一致的等待審查狀態，Agent 應停止 progression、回報 run URL 與狀態，先讓使用者確認 protection 設定是否已改變，不得自行 approve、reject 或 bypass。
- Agent 不得在未經使用者明確要求時新增、移除、放寬或繞過任何 environment protection。
...
- Firestore 上傳需有防護機制；同一使用者或匿名識別來源基準為 60 分鐘內最多 5 次、1 天內最多 10 次，整個 project 另限制 60 分鐘內最多 300 次、1 天內最多 2000 次成功建立。
